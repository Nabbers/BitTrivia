...
const char * const BITCOIN_CONF_FILENAME = "bittrivia.conf";
const char * const BITCOIN_PID_FILENAME = "bittriviad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bittrivia" is a composite category enabling all BitTrivia-related debug output
            if(ptrCategory->count(string("bittrivia"))) {
...
#else
    const char* pszModule = "bittrivia";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitTriviaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitTriviaCore
    // Mac: ~/Library/Application Support/BitTriviaCore
    // Unix: ~/.bittriviacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitTriviaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitTriviaCore";
#else
    // Unix
    return pathRet / ".bittriviacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bittrivia.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bittrivia.conf
